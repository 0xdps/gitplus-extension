...
export async function showFileHistory(uri?: vscode.Uri) {
    ...
    try {
        ...
        if (!logOutput.trim()) {
            vscode.window.showInformationMessage(`No Git history found for ${relativePath}`);
            return;
        }
        ...
        await handleCommitAction(action.value, selected.commit, relativePath, workspacePath);
    } catch (error: any) {
        const errorMessage = error?.message || error?.toString() || 'Unknown error occurred';
        vscode.window.showErrorMessage(`Failed to get file history: ${errorMessage}`);
...
async function showFileContent(hash: string, filePath: string, workspacePath: string) {
    try {
        ...
        vscode.window.showInformationMessage(`Showing file content at commit ${hash.substring(0, 8)}`);
    } catch (error: any) {
        ...
        if (error.message.includes('does not exist') || error.message.includes('No such file')) {
            vscode.window.showWarningMessage(`File does not exist at commit ${hash.substring(0, 8)}. It may have been deleted or renamed.`);
        } else {
            vscode.window.showErrorMessage(`Failed to show file content: ${error.message}`);
        }
    }
}
...
async function showDiff(hash: string, filePath: string, workspacePath: string) {
    try {
        ...
        try {
            ...
            await vscode.window.showTextDocument(doc);
            vscode.window.showInformationMessage(`Showing file addition for commit ${hash.substring(0, 8)} (first commit)`);
            return;
        }
        ...
        await vscode.window.showTextDocument(doc);
        vscode.window.showInformationMessage(`Showing diff for commit ${hash.substring(0, 8)}`);
    } catch (error: any) {
